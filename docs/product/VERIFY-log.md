...
## 2026-07-24 · 0.6.3 · 并存置顶 + 启动取消 + 稳定性

### 范围

- 电影/图片切换窗口不抖（禁 auto InnerSize）
- 后台电影 + 图片：面板可点（强制置顶）
- `play_gen` 取消启动中；工作区失败杀进程
- 开播后尊重置顶开关；托盘单击去重
- config 核心字段 default 防丢片库
- 改平铺屏立即重铺；设置 X 写 Pot 路径
- 置顶/侧栏配置持久化；图片墙文案

### 命令

- [x] `cargo test` — 26 passed / 1 ignored
