...
3. **随机预览** → **开启幻灯**
4. 幻灯：空格暂停，左右切换，Esc 结束  
   平铺墙：点击放大，再点返回，Esc 结束

### 电影 + 图片（并存）

- 电影 **播放中** 再切到 **图片**：PotPlayer **继续播**，应用内可幻灯/平铺墙  
- 图片页会提示「电影仍在后台」；可 **关掉电影**，或切回 **电影** 后用 **关闭本轮**  
- 切回电影会恢复本轮片单与「播放中」状态
...
- `dist/蜜桃成熟-v0.6.0-win64/` — 文件夹（exe + 使用说明）
- `dist/蜜桃成熟-v0.6.0-win64.zip` — 可直接分发
...
- 单实例二次启动唤醒已有窗口
- 索引根目录缓存 + 可取消；模块拆分
- **黑名单**：预览「拉黑」永久排除；设置里可移出
- 电影/图片并存、索引进度、PID 探活等 0.5 能力保留
